...
export default function BlogListingClient({ initialBlogs }: { initialBlogs: BlogPost[] }) {
  ...
  return (
    <section className="bg-white py-10 xlg:py-20 px-4 sm:px-6 md:px-10 xl:px-28 xlg:px-48">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mb-16">
          {blogsToDisplay.map((blog, idx) => (
            <AnimateInView key={idx} direction="up" delay={idx * 0.1}>
  ...
  return (
    <div className="flex justify-center items-center gap-2 flex-wrap">
      ...
      <button
        ...
        className="px-4 py-2 rounded-lg border border-gn text-gn hover:bg-red-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
      >
      ...
      {pagesToShow.map((page) => (
        <button
          ...
          className={`px-4 py-2 rounded-lg border transition-colors duration-200 ${
            page === currentPage
              ? 'bg-red-100 text-gn font-bold border-gn'
              : 'border-gn text-gn hover:bg-red-100 hover:font-bold'
          }`}
        >
          {page}
        </button>
