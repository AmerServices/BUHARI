...
const BlogPostCard = ({ blog }: { blog: BlogPost }) => {
  ...
  return (
    <Link href={href} target={isExternal ? "_blank" : undefined} passHref className="block bg-white rounded-xl shadow-lg overflow-hidden transform hover:scale-105 h-full transition-transform duration-300 group cursor-pointer border border-gray-100 hover:border-gn">
      ...
      <div className="p-6 h-full">
        ...
        <div className="flex justify-between items-center text-sm text-gray-500">
          <span>
            {blogCardText.byLabel} <span className="font-semibold text-gn">{blog.author || "Admin"}</span>
          </span>
