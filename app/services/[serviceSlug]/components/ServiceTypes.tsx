...
const ServiceTypes: React.FC<ServiceTypesProps> = ({ service, serviceType }) => {
  ...
  return (
    ...
    <AnimateInView
      direction="right"
      className="md:sticky top-[144px] lg:m-0 min-w-[200px] bg-white rounded-xl" // Adjusted margin for responsiveness and added shadow/padding
    >
      <nav>
        <ul className="flex flex-col items-center"> {/* Removed justify-between as it's not useful here */}
          {serviceType.map((type) => (
            <li key={type.type} className="w-full mb-3 last:mb-0"> {/* Added margin-bottom */}
              <Link
                href={`https://wa.me/971501113815?text=${encodeURIComponent(`I am interested in the ${type.type} - ${service}`)}`}
                target="_blank"
              ...
              >
                <span className="text-xl font-bold text-center mb-1">{type.type}</span>
